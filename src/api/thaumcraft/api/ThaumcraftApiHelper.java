...
public class ThaumcraftApiHelper {
	...
	public static ItemStack getStackInRowAndColumn(Object instance, int row,
		...
		try {
			Class fake = Class
					.forName("thaumcraft.common.tiles.TileMagicWorkbench");
	...
	public static AspectList getObjectAspects(ItemStack is) {
		...
		try {
			if (getObjectTags == null) {
				Class fake = Class
						.forName("thaumcraft.common.lib.crafting.ThaumcraftCraftingManager");
				getObjectTags = fake
	...
	public static AspectList generateTags(Item item, int meta) {
		try {
			if (generateTags == null) {
				Class fake = Class
						.forName("thaumcraft.common.lib.crafting.ThaumcraftCraftingManager");
				generateTags = fake.getMethod("generateTags", Item.class,
						int.class);
			}
	...
	public static boolean consumeVisFromWand(ItemStack wand,
		...
		try {
			if (consumeVisFromWand == null) {
				Class fake = Class
						.forName("thaumcraft.common.items.wands.ItemWandCasting");
				consumeVisFromWand = fake.getMethod("consumeAllVis",
						ItemStack.class, EntityPlayer.class, AspectList.class,
						boolean.class, boolean.class);
			}
			ot = (Boolean) consumeVisFromWand.invoke(consumeVisFromWand
					.getDeclaringClass().cast(wand.getItem()), wand, player,
					cost, doit, crafting);
		} catch (Exception ex) {
		...
		try {
			if (consumeVisFromWandCrafting == null) {
				...
				consumeVisFromWandCrafting = fake.getMethod(
						"consumeAllVisCrafting", ItemStack.class,
						EntityPlayer.class, AspectList.class, boolean.class);
			}
			ot = (Boolean) consumeVisFromWandCrafting.invoke(
					consumeVisFromWandCrafting.getDeclaringClass().cast(
							wand.getItem()), wand, player, cost, doit);
		} catch (Exception ex) {
	...
	public static boolean consumeVisFromInventory(EntityPlayer player,
		...
		try {
			if (consumeVisFromInventory == null) {
				Class fake = Class
						.forName("thaumcraft.common.items.wands.WandManager");
				consumeVisFromInventory = fake.getMethod(
						"consumeVisFromInventory", EntityPlayer.class,
						AspectList.class);
			}
