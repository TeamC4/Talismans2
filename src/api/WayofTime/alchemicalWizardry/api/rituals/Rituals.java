...
public class Rituals {
	...
	public Rituals(int crystalLevel, int actCost, RitualEffect effect,
			String name) {
	 ...
	 * 
	 * @param key
	 *            Unique identification key - must be different from all others
	 *            to properly register
	 * @param crystalLevel
	 *            Crystal level required to activate
	 * @param actCost
	 *            LP amount required to activate
	 * @param effect
	 *            The effect that will be ticked
	 * @param name
	 *            The name of the ritual
	 * @return Returns true if properly registered, or false if the key is
	 *         already used
	 */
	public static boolean registerRitual(String key, int crystalLevel,
			int actCost, RitualEffect effect, String name) {
	...
	public static boolean checkRitualIsValid(World world, int x, int y, int z,
			String ritualID) {
	 ...
	 */
	public static boolean checkDirectionOfRitualValid(World world, int x,
			int y, int z, String ritualID, int direction) {
		...
		case 1:
			for (RitualComponent rc : ritual) {
				test = world.getBlock(x + rc.getX(), y + rc.getY(),
						z + rc.getZ());
				...
				if (world.getBlockMetadata(x + rc.getX(), y + rc.getY(),
						z + rc.getZ()) != rc.getStoneType()) {
					return false;
				}
			}
		...
		case 2:
			for (RitualComponent rc : ritual) {
				test = world.getBlock(x - rc.getZ(), y + rc.getY(),
						z + rc.getX());
				...
				if (world.getBlockMetadata(x - rc.getZ(), y + rc.getY(),
						z + rc.getX()) != rc.getStoneType()) {
					return false;
				}
			}
	...
	public static int getDirectionOfRitual(World world, int x, int y, int z,
			String ritualID) {
		for (int i = 1; i <= 4; i++) {
			if (Rituals
					.checkDirectionOfRitualValid(world, x, y, z, ritualID, i)) {
				return i;
			}
		}
	...
	public static void performEffect(IMasterRitualStone ritualStone,
			String ritualID) {
		if (ritualMap.containsKey(ritualID)) {
			Rituals ritual = ritualMap.get(ritualID);
			if (ritual != null && ritual.effect != null) {
