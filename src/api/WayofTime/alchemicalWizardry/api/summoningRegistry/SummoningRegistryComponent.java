...
public class SummoningRegistryComponent {
	...
	public SummoningRegistryComponent(SummoningHelper s, ItemStack[] newRing1,
			ItemStack[] newRing2, ItemStack[] newRing3, int amount,
			int bloodOrbLevel) {
	...
	public boolean compareRing(int ring, ItemStack[] checkedRingRecipe) {
		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j = 0; j < 6; j++) {
				...
				if ((checkedItemStack.getItemDamage() == recipeItemStack
						.getItemDamage() || OreDictionary.WILDCARD_VALUE == recipeItemStack
						.getItemDamage())
						&& checkedItemStack.getItem() == recipeItemStack
								.getItem()) {
					test = true;
					checkList[j] = true;
					break;
